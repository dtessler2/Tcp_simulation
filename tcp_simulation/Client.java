...
public class Client {
	...
	public static void main(String[] args) throws IOException {
		...
		if (args.length != 2) {
			System.err.println("Usage: java Client <localhost> <port number>");
			System.exit(1);
		}
			...
			try {
				...
				for (int x = 0; x < packetsReceived.size(); x++) {
					for (int y = x + 1; y < packetsReceived.size(); y++) {
						if (Integer.parseInt(packetsReceived.get(x).substring(0,
								packetsReceived.get(x).indexOf(" "))) > Integer.parseInt(
										packetsReceived.get(y).substring(0, packetsReceived.get(y).indexOf(" ")))) {
